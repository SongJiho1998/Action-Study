...
public class ELServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		/*
		 * 데이터들을 담을 수 있는 JSP 내장객체 종류(scope)
		 * 
		 ...
		 *     한 브라우저당 1개 존재하는 객체
		 *     이 영역에 데이터를 담으면 jsp/servlet 단에서 사용가능하다.
		 *     값이 한번 담기면 서버가 멈추거나, 브라우저가 종료되기전까지 사용가능.
		 *     => 공유범위가 다소 제한적임.
		 *    
		 * 3. HttpServeltRequest (rerquest scope)
		 * 	   요청 및 응답시 매번 생성되는 객체
		 * 	   이 영역에 데이터를 담으면 해당 request객체를 포워딩받는 응답 jsp에서만 사용가능
		 ...
		 *    				 꺼낼때는 getAttribute("키") : 밸류
		 *                   지울때는 removeAttribute("키")
		 *     
		 */
			// requestScope에 데이터 담기
			request.setAttribute("classRoom", "B강의장");
			request.setAttribute("student", new Person("송지호",25,"남자"));
			
			// sessionScope에 담기.
			HttpSession session = request.getSession();
			session.setAttribute("academy", "KH정보교육원");
			session.setAttribute("teacher", new Person("민경민",30,"남자"));
