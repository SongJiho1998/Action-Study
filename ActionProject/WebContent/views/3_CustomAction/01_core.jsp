...
=> c:set 을 통해 선언된 변수는 EL로 접근해서 사용 가능.
...
<!-- 변수명만 제시하면 공유범위가 가장 작은곳부터 큰곳으로 순차적으로 찾아지게됨.
	 티가나지는 않지만 속도가 조금 느려질수있다. 따라서 스코프영역.변수명을 작성하는것을 권장
 -->
${num1} <br>
${num2} <br>
${result} <br>
...
<pre>
* 변수 삭제(&li;c:remove var="제거하고자하는 변수명" scope="스코프영역지정(생략가능)"&gt;)
...
request에 삭제후 result : ${result} <br><br>
...
<c:out value="${outTest}"/> <br> <!-- escapeXml 생략시 기본값은 true == 태그 해석안됨(문자열로 취급) -->
...
<pre>
- JAVA의 if-else, if-else if 또는 switch문과 비슷한 역활을 하는 태그
...
향상된 for문 - (&lt;c:forEach var="변수명" items="순차적으로접근할배열컬렉션" varStatus="현재접근된요소의 상태값을 보관할 변수명(생략가능)"&gt;)
...
<%
	...
	list.add(new Person("송지호", 25 ,"남자"));
	list.add(new Person("송지효", 25 ,"여자"));
...
<%-- 			<% } %> --%>
			<c:choose>
				...
				<c:otherwise>
					<c:forEach var="p" items="${requestScope.pList}" varStatus="status">
						<tr align="center">
							<td>${status.count}</td> <!-- index : 0부터시작, count : 1부터 시작 -->
							<td>${p.name}</td>
							<td>${p.age}</td>
							<td>${p.gender}</td>
						</tr>
	...
	<pre>
	&li;c:forTokens var="각값을 보관할 변수" items="분리시키고자하는문자열" delims="구분자"&gt;
	
	- 구분자를 통해서 분리된 각각의 문자열에 순차적으로 접근하면서 반복 수행
	- Java의 split("구분자") 또는 StringTokenizer와 비슷한 역활
	</pre>
	...
	&lt;c:url var="" value="요청할url"&gt;
		...
		&lt;c:param name="키값" value="밸류값"/&gt;
	&lt;/c:url&gt;
	- url 경로를 생성하고 , 쿼리스트링을 정의할수 있는 태그.
	- 넘겨야할 쿼리스트링이 길경우 사용하면 편리함.
	</pre>
	
	<a href="<%=request.getContextPath() %>/list.do?currentPage=1&num=2">기존 방식</a> <br>
