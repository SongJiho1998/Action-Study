...
<body>
	...
	<p>
		...
		<%--
			teacher에 접근했을때의 밸류값은 Person개체임.
			해당 Person객체의 각 필드에 담긴값을 출력하고자 한다면 .필드명 으로 접근하면됨.
			내부적으로 getter메소드를 찾아서 실해됨(즉, getter메소드는 항상 만들어야함.)
		 --%>
		 수장생 정보
		 <ul>
	...
	sessionScope에 답긴 값 : ${sessionScope.scope} <br>
	applicationScope에 담긴 값 : ${applicationScope.scope} <br><br>
		  
	잘못된 접근시 (session의 classRoom) : ${sessionScope.classRomm}<br>
